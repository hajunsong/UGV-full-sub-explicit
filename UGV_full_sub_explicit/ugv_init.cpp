...
void UnmannedGroundVehicle::init() {
	CsvParser *csvparser;
	CsvRow *row;

	csvparser = CsvParser_new("car_sim_path_filtered.csv", ",", 0);
	int row_index = 1;
	int col_index = 1;
	...
	while ((row = CsvParser_getRow(csvparser))) {
		...
		for (int i = 0; i < CsvParser_getNumFields(row); i++) {
			if (!(strcmp(rowFields[i], ""))) break;
			else if (!(strcmp(rowFields[i], "waypoint_size"))) {
				input->WaypointSize = atoi(rowFields[i + 1]);
				input->LocalPath = new double*[input->WaypointSize];
				for (int i = 0; i < input->WaypointSize; i++) input->LocalPath[i] = new double[2];
			}
			if (row_index >= 2) {
				input->LocalPath[row_index - 2][0] = atof(rowFields[i]);
				input->LocalPath[row_index - 2][1] = atof(rowFields[i + 1]);
				break;
			}
			col_index++;
		}
	...
	/////////// Path //////////
	wp_size = input->WaypointSize;	// 입력 받은 waypoint size를 RTT class 내부에서 사용하는 변수에 저장

	// initial heading direction
	heading = atan2((input->LocalPath[1][1] - input->LocalPath[0][1]), (input->LocalPath[1][0] - input->LocalPath[0][0]));
